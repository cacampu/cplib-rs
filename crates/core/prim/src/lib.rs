/// プリミティブ整数型を相互変換する識別トレイト。
/// `usize`, `i32` など言語仕様上は別型として存在するものを ℤ の元として同一視する。
///
/// 変換は `as` キャストなので、表現できない値は切り詰め・ラップアラウンドする。
pub trait Int: Copy {
...
pub trait IntN<const N: usize>: Sized {
    ...
    #[inline]
    fn to_usizes(self) -> [usize; N] {
        self.to_isizes().map(|c| c as usize)
    }
